...
router.get('/:id/queue-status', authMiddleware, async (req, res) => {
  try {
    ...
    const waitingAhead = queueAhead || 0;
    const avgMinutes = 30; // ค่าเฉลี่ยต่อคิว (นาที) — TODO: ดึงจาก config หรือ product_slots

    // [FIX] currentQueue = คิวแรกสุดของ slot ที่ยัง waiting อยู่จริง (คิวที่กำลังให้บริการ)
    // ไม่ใช่ queue_no ของ booking ตัวเอง
    let currentQueueNo = '-';
    if (booking.slot_id) {
      const { data: firstWaiting } = await supabaseAdmin
        ...
        .limit(1)
        .maybeSingle();
      currentQueueNo = firstWaiting?.queue_no || '-';
    }
    ...
    res.json({
      ...
      data: {
        ...
        queue_position:      waitingAhead + 1,   // ตำแหน่งในคิว (เริ่มจาก 1)
        // [FIX] currentQueue = คิวแรกที่กำลังให้บริการอยู่จริงใน slot นี้
        currentQueue:        currentQueueNo,
        waitingAhead:        waitingAhead,        // จำนวนคิวที่รออยู่ก่อนหน้า
        estimatedMinutes:    waitingAhead * avgMinutes,
        averageTimePerQueue: avgMinutes,
        slot:                booking.slot,
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (slot_id) {
      ...
      const { data: rpcSeq, error: rpcSeqErr } = await supabaseAdmin
        .rpc('next_queue_sequence', { p_slot_id: slot_id });
    ...
    await supabaseAdmin
      ...
      .insert({
        ...
        type: 'booking',
        title: 'มีการจองคิวใหม่',
        description: `มีรายการจองใหม่เลขที่ ${booking_no} (${queue_no})`,
        is_read: false
      })
      .then(() => null)
      .catch(() => null);
 ...
 * อัปเดตสถานะการจอง
 * [FIXED] ตรวจสอบ ownership ก่อนอนุญาตให้แก้ไข
 *   - farmer เจ้าของ: ยกเลิก (cancel) ได้เท่านั้น
 *   - buyer เจ้าของ: อนุมัติ (success) หรือยกเลิก (cancel) ได้
 */
router.patch('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // ตรวจสอบสิทธิ์: ต้องเป็น farmer หรือ buyer เจ้าของรายการนี้เท่านั้น
    const isFarmerOwner = role === 'farmer' && booking.farmer_id === userId;
    const isBuyerOwner  = role === 'buyer'  && booking.buyer_id  === userId;

    if (!isFarmerOwner && !isBuyerOwner) {
      return res.status(403).json(response.error('คุณไม่มีสิทธิ์แก้ไขรายการจองนี้'));
    }

    // farmer ยกเลิกได้อย่างเดียว (ไม่สามารถอนุมัติตัวเองได้)
    if (isFarmerOwner && status === 'success') {
      return res.status(403).json(response.error('Farmer ไม่สามารถอนุมัติการจองได้ด้วยตัวเอง'));
    }
 ...
 * [FIXED] ตรวจสอบ ownership ก่อนยกเลิก
 *   - farmer เจ้าของเท่านั้นที่ยกเลิกคิวตัวเองได้
 */
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    // ตรวจสอบสิทธิ์: ต้องเป็น farmer หรือ buyer เจ้าของรายการนี้
    const isFarmerOwner = role === 'farmer' && booking.farmer_id === userId;
    const isBuyerOwner  = role === 'buyer'  && booking.buyer_id  === userId;

    if (!isFarmerOwner && !isBuyerOwner) {
      return res.status(403).json(response.error('คุณไม่มีสิทธิ์ยกเลิกรายการจองนี้'));
    }

    if (booking.status === 'cancel') {
      return res.status(400).json(response.error('รายการนี้ถูกยกเลิกไปแล้ว'));
    }
