...
router.post('/:roomId/messages', authMiddleware, upload.single('image'), async (req, res) => {
  try {
    ...
    // สร้าง notification ให้คู่สนทนาเมื่อมีข้อความใหม่
    ...
    const receiverId = room
      ? (room.user1_id === req.user.id ? room.user2_id : room.user1_id)
      : null;

    if (receiverId) {
      const text = String(message || '').trim();
      const preview = text || (req.file ? 'ส่งรูปภาพ' : 'มีข้อความใหม่');
      await supabaseAdmin
        .from('notifications')
        .insert({
          user_id: receiverId,
...
router.delete('/:roomId', authMiddleware, async (req, res) => {
  try {
    ...
    // ตรวจสอบก่อนว่าผู้ใช้เป็นสมาชิกในห้องแชทนี้หรือไม่
    ...
    if (roomError || !room) return res.status(404).json(response.error('ไม่พบห้องแชท'));
    if (room.user1_id !== uid && room.user2_id !== uid) {
      return res.status(403).json(response.error('คุณไม่มีสิทธิ์ลบห้องแชทนี้'));
    }
    ...
    res.json(response.success('ลบการสนทนาเรียบร้อยแล้ว'));
  } catch (e) {
    res.status(500).json(response.error(e.message));
  }
});
