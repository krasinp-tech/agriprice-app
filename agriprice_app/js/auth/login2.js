/* js/login/register/login2.js
   ...
   - Else: mock login (0902866361 / n12345)
...
(function () {
  ...
  // mock account (ทดสอบได้ทันที)
  const MOCKS = [
    {
      username: "0902866361",
      password: "n12345",
      user: { id: 1, phone: "0902866361", role: "farmer", name: "Agriprice Farmer" },
      token: "mock_farmer_token_" + Date.now(),
    },
    // เผื่อทดสอบ role buyer (ไม่แสดงใน UI)
    {
      username: "0800000000",
      password: "b12345",
      user: { id: 2, phone: "0800000000", role: "buyer", name: "Agriprice Buyer" },
  ...
  function persistAuth(token, user) {
    ...
    // ✅ บันทึก role เแยกเพื่อให้ components เข้าถึงงาย
    if (user && user.role) {
      localStorage.setItem("role", String(user.role).toLowerCase());
    } else {
      // ถ้าไม่มี role ให้เป็น guest/empty
  ...
  function redirectAfterLogin() {
    ...
    const target = next || stored || CONFIG.DEFAULT_HOME;
  ...
  async function loginMock(identifier, password) {
    const idDigits = (identifier || "").replace(/[^\d]/g, "");

    const found = MOCKS.find(m =>
      (identifier === m.username || idDigits === m.username) && password === m.password
    );

    if (!found) throw new Error("อีเมล/เบอร์โทร หรือรหัสผ่านไม่ถูกต้อง");
  ...
  async function onSubmit(e) {
    ...
    try {
      const result = CONFIG.API_BASE
        ? await loginViaApi(identifier, password)
        : await loginMock(identifier, password);
