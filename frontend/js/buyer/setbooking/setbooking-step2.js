...
(function () {
  ...
  async function apiCall(method, path, body) {
    ...
    if (path.startsWith("/api/products") && !isGet) {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message || (method === "POST" ? "สร้างสินค้าไม่สำเร็จ" : "แก้ไขสินค้าไม่สำเร็จ"));
      }
  ...
  saveAllBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        productResult = await apiCall('PATCH', `/api/products/${productIdFromStep1}`, productData);
      } else {
      ...
      window.appNotify(isEdit ? "อัปเดตการรับซื้อสำเร็จ!" : "ลงทะเบียนรับซื้อสำเร็จ!", "success");
