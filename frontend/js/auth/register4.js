...
(function () {
  ...
  const USER_KEY  = window.AUTH_USER_KEY  || 'user_data';
  ...
  function cleanup() {
    [KEY_ROLE, KEY_PROFILE, KEY_TEMP_TOKEN, 'otp_bypass'].forEach(k => sessionStorage.removeItem(k));
  }
  ...
  async function doRegister(temp_token, role, profile, password) {
    ...
    // Sanitize profile: 
    // 1. Only keep: first_name, last_name, email (if valid), phone
    ...
    const res = await fetch(currentBase + '/api/auth/register/finish', {
      method: 'POST',
  ...
  form?.addEventListener('submit', async e => {
    ...
    if (!temp_token && getApiBase()) { showErr('OTP token หมดอายุ กรุณายืนยัน OTP ใหม่'); return; }
    try {
