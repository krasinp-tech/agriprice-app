...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function myUserId() {
    try { return JSON.parse(localStorage.getItem(window.AUTH_USER_KEY || 'user_data') || 'null')?.id || null; }
    catch (_) { return null; }
  }
  ...
  function normalizeProfileImageUrl(raw) {
    ...
    if (!value.includes('/')) return resolveAssetPath('assets/images/' + value); // Handle bare filenames like 'avatar-buyer.svg'
    return value;
  }
  ...
  const ChatAPI = {
    async listConversations() {
      ...
      const currentBase = getApiBase();
      if (!currentBase) {
        if (DEBUG_CHAT) console.warn('[chat] getApiBase() not configured');
        return [];
      }
      try {
        ...
        const res = await fetch(currentBase + '/api/chats', { headers: authHeaders() });
    ...
    async getMessages(chatId) {
      ...
      try {
        const res = await fetch(currentBase + '/api/chats/' + chatId + '/messages', { headers: authHeaders() });
        ...
        return (json.data || []).map(m => ({
          ...
          text: m.message || '',
          imageUrl: normalizeProfileImageUrl(m.image_url),
    ...
    async sendMessage(chatId, { text, contentType, imageData, imageFile }) {
      ...
      try {
        ...
        const res = await fetch(currentBase + '/api/chats/' + chatId + '/messages', { method: 'POST', headers, body });
        if (!res.ok) throw new Error(res.status);
        const json = await res.json();
        const m = json.data || {};
        return {
    ...
    async deleteConversation(chatId) {
      if (window.APP_CONFIG_READY) await window.APP_CONFIG_READY;
      const currentBase = getApiBase();
      if (!currentBase || !chatId) return false;
      try {
        const res = await fetch(currentBase + '/api/chats/' + chatId, {
          method: 'DELETE',
          headers: authHeaders(),
        });
        return res.ok;
  ...
  function setMobileRoomMode(isOpen) {
    ...
    // ซ่อน/แสดง Bottom Nav เมื่อเข้าห้องแชท
    const bottomNav = document.getElementById('bottomNavMount') || document.getElementById('bottomNav');
    if (bottomNav) {
  ...
  function renderList(list) {
    ...
    list.forEach((c) => {
      ...
      el.innerHTML = `
        <button class="chat-item-delete-btn" type="button" data-action="swipe-delete" data-chat-id="${c.chatId}">
  ...
  function initSwipeToDelete() {
    ...
    items.forEach(item => {
      ...
      item.addEventListener('touchmove', (e) => {
        ...
        if (diff > 0) diff = 0;
        if (diff < -DELETE_BTN_WIDTH - 20) diff = -DELETE_BTN_WIDTH - 20; // limit pull
      ...
      item.addEventListener('touchend', () => {
        ...
        if (currentX < -DELETE_BTN_WIDTH / 2) {
          content.style.transform = `translateX(-${DELETE_BTN_WIDTH}px)`;
          isOpen = true;
        } else {
      ...
      // Close other swiped items when clicking
      item.addEventListener('click', (e) => {
        if (isOpen && !e.target.closest('.chat-item-delete-btn')) {
          e.preventDefault();
          e.stopPropagation();
          content.style.transform = 'translateX(0)';
          isOpen = false;
  ...
  function showRoom(convo, msgs) {
    ...
    if (btnCall) {
      btnCall.onclick = () => {
        ...
        } else if (getApiBase() && convo.sellerId) {
          fetch(`${getApiBase()}/api/profiles/${convo.sellerId}`, { headers: authHeaders() })
    ...
    roomMessages.innerHTML = msgs.map((m) => {
      ...
      if (m.contentType === "image" && (m.imageData || m.imageUrl)) {
        ...
        const textBubble = m.text ? `<div class="bubble-text-part">${escapeHtml(m.text)}</div>` : "";
        contentHtml = `
          <div class="bubble image-bubble">
            ${textBubble}
            <img src="${escapeHtml(imgSrc)}" class="bubble-image" alt="${escapeHtml(t('attach_image', 'แนบรูป'))}" />
          </div>`;
      } else if (m.text) {
  ...
  listMount?.addEventListener("click", async (e) => {
    ...
    const delBtn = e.target.closest('[data-action="swipe-delete"]');
    if (delBtn) {
      e.preventDefault();
      e.stopPropagation();
      const chatId = delBtn.dataset.chatId;
      const confirmed = confirm(t('chat_delete_confirm', 'คุณต้องการลบการสนทนานี้และข้อความทั้งหมดใช่หรือไม่?'));
      if (!confirmed) return;

      const success = await ChatAPI.deleteConversation(chatId);
      if (success) {
        window.appNotify(t('chat_deleted', 'ลบการสนทนาเรียบร้อยแล้ว'), 'success');
        await loadConversations();
        if (activeChatId === chatId) closeRoomOnMobile();
      } else {
        window.appNotify(t('chat_delete_failed', 'ลบการสนทนาไม่สำเร็จ'), 'error');
      }
      return;
    }
  ...
  roomHeader?.addEventListener("click", async (e) => {
    ...
    if (action === "call") {
      ...
      if (phone) {
        // Dial on mobile.
        if (window.navigateWithTransition) window.navigateWithTransition(`tel:${phone.replace(/[^0-9+]/g, '')}`); else window.location.href = `tel:${phone.replace(/[^0-9+]/g, '')}`;
      } else if (getApiBase() && activeConversation?.sellerId) {
        // Load phone from API.
        fetch(`${getApiBase()}/api/profiles/${activeConversation.sellerId}`, { headers: authHeaders() })
          .then(r => r.ok ? r.json() : null)
          .then(p => {
            const tel = p?.phone || '';
            if (tel) {
  ...
  // ----------------------------
  // Init: read ?chatId= or ?targetId= from URL
  // ----------------------------
  async function init() {
    await loadConversations();
    const params = new URLSearchParams(window.location.search);
    ...
    if (urlTargetId) {
      if (DEBUG_CHAT) console.log('[chat] Target ID found:', urlTargetId);
      try {
        const res = await fetch(getApiBase() + '/api/chats/start', {
          method: 'POST',
          headers: { ...authHeaders(), 'Content-Type': 'application/json' },
          body: JSON.stringify({ target_user_id: urlTargetId })
        });
        const json = await res.json();
        if (json.success && json.data.room_id) {
          const newRoomId = json.data.room_id;
          await loadConversations(); // Refresh list to include new room
          await openChat(newRoomId);
        }
      } catch (err) {
        console.error('[chat] Failed to start chat with targetId:', err);
      }
    } else if (urlChatId) {
      ...
      if (existing) {
        ...
        try {
          const roomRes = await fetch(`${getApiBase()}/api/chats`, { headers: authHeaders() });
          if (roomRes.ok) {
  ...
  async function startHeartbeat() {
    ...
    const ping = () => fetch(getApiBase() + '/api/presence/ping', {
  ...
  async function updateOnlineStatus(userId) {
    if (!userId || !getApiBase()) return;
    try {
      const res = await fetch(`${getApiBase()}/api/presence/${userId}`, { headers: authHeaders() });
  ...
  async function updateUnreadBadge() {
    ...
    try {
      const res = await fetch(getApiBase() + '/api/chats/unread', { headers: authHeaders() });
      if (!res.ok) {
        clearBadge();
        return;
      }
