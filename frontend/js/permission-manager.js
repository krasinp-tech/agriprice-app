...
(function () {
  ...
  async function requestLocation(options = {}) {
    // [UNLOCKED] We always check the real OS status now for the presentation
  ...
  async function requestCamera() {
    const prefs = getPrefs();
    // Allow re-requesting even if previously denied to improve UX
  ...
  async function requestAll() {
    // [UNLOCKED] Always attempt to request permissions during the audit phase

    // Check current states before asking to be extra safe
    if (isNative()) {
      try {
        const Geo = getPlugin('Geolocation');
        const Notif = getPlugin('LocalNotifications');
        const geoPerm = Geo ? await Geo.checkPermissions() : { location: 'denied' };
        const notifPerm = Notif ? await Notif.checkPermissions() : { display: 'denied' };

        // If already decided by OS, don't trigger request
        if (geoPerm.location !== 'prompt' && notifPerm.display !== 'prompt') {
          setPref('asked_all_v3', true);
          return;
        }
      } catch (e) {}
    }
    ...
    // Mark as "Asked" so we never disturb the user again
    setPref('asked_all_v3', true);
  }
