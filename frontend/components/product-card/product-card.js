...
(function () {
  ...
  function createCardEl(data, handlers, templateHtml) {
    ...
    // [ADDED] Translate static labels in the template
    if (window.i18nT) {
      card.querySelectorAll('[data-i18n]').forEach(el => {
        const key = el.getAttribute('data-i18n');
        el.textContent = window.i18nT(key, el.textContent);
      });
      card.querySelectorAll('[data-i18n-placeholder]').forEach(el => {
        const key = el.getAttribute('data-i18n-placeholder');
        el.setAttribute('placeholder', window.i18nT(key, el.getAttribute('placeholder')));
      });
      card.querySelectorAll('[data-i18n-alt]').forEach(el => {
        const key = el.getAttribute('data-i18n-alt');
        el.setAttribute('alt', window.i18nT(key, el.getAttribute('alt')));
      });
    }
