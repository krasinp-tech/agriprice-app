(function initHeroSlider() {
  ...
  carousel.addEventListener("pointercancel", endDrag);
    ...
    function renderCategories(categories) {
      // Mock fruit categories (order and fill)
      const mockOrder = [
      ...
      // Fill with mock if missing
      const fullCats = mockOrder.map(name => catMap[name] || { id: name, name });
      // Clear grid
      track.innerHTML = '';
...
(async function initProductCards() {
  ...
  try {
    ...
    // 2) ดึงข้อมูลจาก API จริง (fallback เป็น mock ถ้าไม่มี server)
    let products = [];
    const API_BASE = (window.API_BASE_URL || '').replace(/\/$/, '');
    ...
    if (API_BASE) {
      try {
        const token = localStorage.getItem(window.AUTH_TOKEN_KEY || 'token') || '';
        const apiRes = await fetch(API_BASE + '/api/products?limit=50', {
        ...
        if (apiRes.ok) {
          ...
          const allProducts = rows.map(p => {
            ...
            // ดึง grades จาก product_grades ถ้ามี
          ...
          products = allProducts.slice(0, 3); // แสดงแค่ 3 อันแรก
          // เก็บไว้ใน mount เพื่อ load more
