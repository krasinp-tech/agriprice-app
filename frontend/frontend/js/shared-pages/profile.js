document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (API_BASE && uid) {
    try {
      ...
      if (revRes.ok) {
        ...
        apiReviews = (revJson.data || []).map(r => ({
          name:   r.reviewer ? `${r.reviewer.first_name} ${r.reviewer.last_name}`.trim() : 'ไม่ทราบชื่อ',
  ...
  set('reviewCount', `(${apiReviews.length})`);
  ...
  if (reviewContainer) {
    ...
    if (!apiReviews.length) {
      showEmptyState(reviewContainer, 'ยังไม่มีรีวิว');
    } else {
      hideEmptyState(reviewContainer);
      apiReviews.forEach(r => {
        ...
        item.innerHTML = `
          <div class="review-header">
            <div class="reviewer-avatar"><span class="material-icons-outlined" aria-hidden="true">person</span></div>
            <div>
