...
async function seed() {
  ...
  for (let i = 0; i < 5; i++) {
    const phone = `081000000${i + 1}`;
    const email = `testuser${i + 1}@agriprice.com`;
    ...
    try {
      ...
      // 2. Update Profile (Profile is usually created via trigger, but we ensure values here)
      ...
      console.log(`[OK] Profile updated with location: ${loc.name}`);
